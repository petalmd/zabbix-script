...
CACHETTL="25" # Cache duration in seconds
CACHE="/tmp/ceph-status.cache"
...
if [ "$(($TIMENOW - $TIMECACHE))" -gt "$CACHETTL" ]; then
  ...
  status_txt=$($ceph_bin health | awk '{print $1}')
  case $status_txt in
    HEALTH_OK)
      status=1
    ;;
  ...
  mon=$(ceph_mon_get_active)
  up=$(ceph_osd_up_percent)
  in=$(ceph_osd_in_percent)

  echo "
health $status
rados_total $rados_total
rados_used $rados_used
rados_free $rados_free
mon $mon
count $ceph_osd_count
up $up
in $in
degraded_percent $pggdegraded
pgtotal $pgtotal
creating $creating
active $active
clean $clean
